...
public class TPPSQLWrapperInsertServerStorageLocationTest {
    ...
    public void beforeEach() throws SQLException {
        ...
        when(this.connection.prepareStatement("INSERT INTO tpp_server_storage_locations (storage_name, effective_storage_name, loc_x, loc_y, loc_z, world_name) VALUES (?, ?, ?, ?, ?, ?)")).thenReturn(this.preparedStatement);
        when(this.preparedStatement.executeUpdate()).thenReturn(1);
    }

    @Test
    @DisplayName("insertServerStorageLocation returns true")
    void insertServerStorageLocationReturnsTrue() throws SQLException {
        assertTrue(this.mockSQLWrapper.insertServerStorageLocation("StorageName", new Location(this.world, 1, 2, 3)));
    ...
    @Test
    @DisplayName("insertServerStorageLocation returns false when no row inserted")
    void insertServerStorageLocationReturnsFalse() throws SQLException {
        when(this.preparedStatement.executeUpdate()).thenReturn(0);

        assertFalse(this.mockSQLWrapper.insertServerStorageLocation("StorageName", new Location(this.world, 1, 2, 3)));
    ...
    @Test
    @DisplayName("insertServerStorageLocation rethrows exceptions")
    void insertServerStorageLocationRethrowsExceptions() throws SQLException {
        when(this.preparedStatement.executeUpdate()).thenThrow(new SQLException());

        assertThrows(SQLException.class, () -> this.mockSQLWrapper.insertServerStorageLocation("StorageName", new Location(this.world, 1, 2, 3)));
    ...
    void insertLostRegionReturnsFalseNoWorld() throws SQLException {
        assertFalse(this.mockSQLWrapper.insertServerStorageLocation("StorageName", new Location(null, 1, 2, 3)));
